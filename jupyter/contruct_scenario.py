...
def transform_obs_car(obs_car, heading, y_offset, x_offset_adjustment):
    """Apply transformations to the obstacle car's coordinates."""
...
def construct_scenario(
    ...
    if is_front_occupied:
        front_x_vec, front_y_vec = transform_obs_car(
            obs_car,
            front_car_heading,
            front_car_y_offset,
            slot_length + obs_to_slot_line,
        )
    ...
    rear_obs_car_matrix = []
    if is_rear_occupied:
        rear_x_vec, rear_y_vec = transform_obs_car(
            obs_car, rear_car_heading, rear_car_y_offset, -obs_to_slot_line
        )
        max_x = max(rear_x_vec)
        rear_x_vec = [x - max_x - obs_to_slot_line for x in rear_x_vec]
    ...
    for x_offset, y_offset, heading_offset in zip(
        x_offset_vec, y_offset_vec, heading_offset_vec
    ):
        ...
        channel_y_vec = [y - min_channel_car_y + half_slot_width + channel_width for y in channel_y_vec]
